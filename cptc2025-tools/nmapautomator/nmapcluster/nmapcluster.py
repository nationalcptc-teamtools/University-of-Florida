    ...
    while True:
        ...
        for scan_result in scan_results_list:
            ...
            if scan_result[0].scan_purpose == 2:
                ...
                # TODO Put the scan output and the XML string into the database.
                
                database: sqlite3.Connection = sqlite3.connect("openports.db")
                for scanned_host in scan_output.hosts:
                    database.execute("CREATE TABLE IF NOT EXISTS \"ports\" ( \"host\" TEXT NOT NULL, " + 
                            "\"port\" INTEGER NOT NULL, \"protocol\" TEXT NOT NULL, \"service\" TEXT, PRIMARY KEY(\"host\",\"port\",\"protocol\"));")
                    database.execute("CREATE TABLE IF NOT EXISTS \"scans\" ( \"fid\" INTEGER NOT NULL UNIQUE, " + 
                            "\"host\" TEXT NOT NULL, \"rawScan\" TEXT, \"xmlScan\" TEXT, PRIMARY KEY(\"fid\" AUTOINCREMENT));")
                    database.execute("INSERT INTO scans (host, rawScan, xmlScan) VALUES (?, ?, ?)", 
                            (scanned_host.ip, scan_result[0].scan_output, scan_result[1]))
                    for port in scanned_host.open_ports:
                        database.execute("INSERT OR IGNORE INTO ports (host, port, protocol, service) " + 
                                "VALUES (?, ?, ?, ?)", (scanned_host.ip, port.number, port.protocol, port.service))
                database.commit()
